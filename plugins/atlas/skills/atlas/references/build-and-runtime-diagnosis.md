...
Separate source/type errors, generated-artifact mismatch, dependency resolution, environment/configuration and runtime data failures. Reproduce the narrowest representative failure, then test a hypothesis that discriminates between causes. For a repeatable source or configuration failure, repeating the same action is not a new investigation. Safe retries for credible transient failures follow [delivery and repair](deliver-and-repair.md); a second failure alone is not a stop rule. Check uncertain external effects through [failure handling](failure-handling.md#decide-whether-repetition-is-safe) before retrying. Keep effort bounded by the task’s authority, time and risk; do not restart an unlimited loop or shrink the requested outcome.
