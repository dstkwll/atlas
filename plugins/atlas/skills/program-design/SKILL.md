...
**Bounded proof:** plan tests around accepted behavior classes, invariants, authority boundaries, and canonical transitions. Do not multiply tests across prose variants or let hypothetical edge cases create implementation seams. Every test seam must map to an accepted requirement, a necessary invariant or authority boundary implied by accepted design, or a reachable failure class; validation cost alone does not justify a new abstraction.
