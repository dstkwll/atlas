...
If the profile is unavailable, invoke the Atlas plugin's skill from the `/` picker (it may appear as `/atlas` or `/atlas:atlas`) or ask: **Use the Atlas skill from the atlas-successor plugin for this task: [goal].**
...
See the repository [setup guide](https://github.com/dstkwll/atlas-successor/blob/main/SETUP.md) for installation, updates, manual skill use, continuity and migration. All operational guidance is contained in [the skill](skills/atlas/SKILL.md) and its relative references; the profile is only a native entrypoint. No access to the Atlas repository or Drive is required during work.
