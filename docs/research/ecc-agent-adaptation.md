...
The September 2026 donor assessment also drew bounded inspiration from [HumanLayer's advanced context engineering essays](https://github.com/humanlayer/advanced-context-engineering-for-coding-agents/tree/f2bc7aec4575418d2d2e83fec078266cc56d3e6a), [Masterplan](https://github.com/rasatpetabit/masterplan/tree/8db2d9afe6d1dfa161197c952c849501688c94ec), [Groundwork](https://github.com/tony-adamson/groundwork/tree/2385c49f6bd52496298d173a55d52d2345e4e521), [PlanF3](https://github.com/disler/planf3/tree/f34b7ba5ff8167ef6283791763f4b578bfb3a2c0), [Warren](https://github.com/jayminwest/warren/tree/2b770c2f87b5e576d80c1f5ff604ca2f92961d27), [AutoPrompt](https://github.com/Spielewoy/autoprompt-skill/tree/91dc6edf336abea96ceb10eaf7efab77ca4c7007) and [Sandcastle](https://github.com/mattpocock/sandcastle/tree/e99f832f26dc9d245c019a9ddd19fa5dee792427). Atlas adapts selective code-shape sketches, intent reconciliation, marginal-value discovery stopping, slow-resource failure analysis, interruption recovery, issue-first review and explicit isolation distinctions into existing guides. [Ringer](https://github.com/NateBJones-Projects/ringer/tree/5dfad1460385ff2d4e4ac156accbc6eb3ad33f41) prompted only generic attention to informative failures and whole-task cost; no code or prose was copied because its license is restrictive. Fixed stages, mandatory agent systems, automatic integration and new runtime dependencies remain excluded.
...
The PRD, ticket and handoff entry points share Atlas runbooks rather than import a donor workflow. The same inspected Matt Pocock revision above supplied `to-tickets`, `codebase-design`, `DESIGN-IT-TWICE`, `improve-codebase-architecture`, `tdd` and `handoff`. Adapted techniques are dependency-aware vertical slices, explicit handling of wide migrations, caller-grounded interface design, behavioral tests in small increments and recipient-oriented context. Existing Atlas discovery guidance owns PRD content and acceptance. No donor `to-prd` source was required.

The installed ECC `contract-first` and `blueprint` sources informed shared consumer/provider contracts and fresh-recipient execution context. Atlas does not import mandatory setup, vocabulary, fixed phases/model counts, automatic issue publication or acceptance labels, universal test-boundary approvals, fixed coverage thresholds, temporary-only handoffs or speculative infrastructure. Atlas keeps red-green-refactor when appropriate rather than importing the donor's exclusion of refactoring from the loop. Readiness distinguishes unresolved judgment, environment prerequisites and known dependency outputs; a graph with no blockers is not execution authority. Relevant source paths are retained here only; no donor runtime or installation dependency is added.

## Investigation and verification methods

The [pstack assessment](https://github.com/cursor/plugins/tree/9bd4a8289f1c3fe870d518051772762a78b66ea0/pstack) informed independently written Atlas guidance for historical rationale, project verification recipes, issue intake and existing-fix checks, faithful interactive evidence, captured profiles, measured optimization, visual preservation, focused assurance, guidance diagnosis and reader-oriented explanation. Two optional references hold the distinct rationale and verification-recipe methods; other refinements live in existing runbooks. Source code, templates, illustrations and donor prose are not included.

The adaptations retain current authority, project-owned artifacts and claim-specific proof. They exclude fixed panels, universal verification counts, mandatory delegation, new runtime dependencies, automatic publication and copied operational policy. Candidate-comparison experiments and standalone status/cleanup helpers remain future options requiring a concrete task; no new public skill or automated donor synchronization is introduced.
