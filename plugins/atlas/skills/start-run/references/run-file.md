...
- `governance` records the accepted posture; `gates` records the fully resolved policy for every selected stage and every run-relevant conditionally reachable route. Every entry has `authority`. Valid authorities are `AUTO`, `AGENT_REVIEW`, `HUMAN`, `CONDITIONAL`, and `HUMAN_IF_CHANGED`. The explicit map wins over later changes to global profiles.
- A conditionally reachable route omitted from `stages` records `activation.when` separately from gate review policy. Its mutable gate uses canonical `NOT_REQUIRED` while inactive; persisted activation evidence makes it reachable, after which `conditions` select review authority. The activation predicate—not a new state label—preserves its reachability.
