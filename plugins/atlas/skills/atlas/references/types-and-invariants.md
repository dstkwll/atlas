...
When an input appears broader than necessary, inspect production construction paths and consumers alongside accepted requirements and public compatibility. Separate optional input from a required input whose value may legitimately be null. Fixtures, stories and mocks should satisfy the supported contract; their convenience alone does not justify optional callbacks or fallback states in production. Conversely, a test may encode required behavior that current callers do not yet exercise. Missing local usage is insufficient evidence to remove a supported capability, especially when external consumers are inaccessible. Narrow only with evidence for the contract, adapt support code accordingly, and verify affected consumers without casts or weakened tests hiding a mismatch.
