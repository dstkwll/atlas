...
For collaborative design, add **Work through the design with me before implementing.** For delivery, explicitly ask for implementation and specify the desired result and constraints. Atlas chooses the relevant runbooks. For substantive work, expect a brief notice naming the guide being applied and its purpose, followed by work informed by it. An Atlas load event alone does not establish runbook use; a simple answer or deterministic small edit may legitimately need no guide. No calibration report or workplace evidence export is needed.
